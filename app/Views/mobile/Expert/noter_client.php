...
$errors      = $errors ?? [];
$clientNom   = trim(($reservation['client_prenom'] ?? $reservation['prenom'] ?? '') . ' ' . ($reservation['client_nom'] ?? $reservation['nom'] ?? ''));
?>
...
<div style="display:flex;align-items:center;gap:0.75rem;margin-bottom:1.5rem">
    ...
    <div>
        <h1 style="margin:0;font-size:1.1rem;font-weight:700;color:var(--primary)">Noter le client</h1>
        <?php if ($clientNom): ?>
        <p style="margin:0;font-size:0.78rem;color:var(--text-muted)"><?= $e($clientNom) ?></p>
        <?php endif; ?>
    </div>
</div>
...
<div style="background:var(--card-bg);border:1px solid var(--border);border-radius:var(--radius);padding:1.25rem;margin-bottom:1rem">
    <form method="post" action="<?= $baseUrl ?>/expert/noter-client/<?= (int)($reservation['id'] ?? 0) ?>" class="form-mobile">
        <?= $csrfField ?>

        <!-- Étoiles -->
        <div style="margin-bottom:1.25rem">
            <label style="display:block;font-size:0.85rem;font-weight:600;color:var(--text);margin-bottom:0.75rem;text-align:center">
                Note (1 à 5 étoiles) <span style="color:#dc2626">*</span>
            </label>
            <div style="display:flex;justify-content:center;gap:0.5rem">
                <?php for ($i = 1; $i <= 5; $i++): ?>
                <button type="button" class="rating-star-client" data-value="<?= $i ?>"
            ...
            <input type="hidden" name="note" id="note-client" value="" required>
            <p style="text-align:center;margin:0.5rem 0 0;font-size:0.78rem;color:var(--text-muted)" id="note-client-label">Touchez une étoile</p>
        </div>
        ...
        <a href="<?= $baseUrl ?>/expert/reservations" class="btn-mobile btn-outline" style="display:flex;align-items:center;justify-content:center">Annuler</a>
    </form>
</div>

<script>
(function(){
    var stars  = document.querySelectorAll('.rating-star-client');
    var input  = document.getElementById('note-client');
    var label  = document.getElementById('note-client-label');
    ...
    stars.forEach(function(btn) {
        ...
        btn.addEventListener('mouseover', function() { highlight(parseInt(btn.getAttribute('data-value'), 10)); });
        btn.addEventListener('mouseout', function() { highlight(parseInt(input.value || '0', 10)); });
    });
})();
</script>
