...
<div style="margin-bottom:1.25rem">
    <h1 style="margin:0;font-size:1.2rem;font-weight:700;color:var(--primary)">Mes commandes</h1>
...
<div style="text-align:center;padding:3rem 1rem">
    ...
    <p class="mobile-empty-hint">Aucune commande pour le moment.</p>
...
<div style="display:flex;flex-direction:column;gap:0.75rem">
    ...
    <div style="background:var(--card-bg);border:1px solid var(--border);border-left:3px solid <?= $sc ?>;border-radius:var(--radius);box-shadow:var(--shadow);overflow:hidden">
           ...
           style="display:block;text-decoration:none;color:inherit;padding:1rem;padding-bottom:<?= (($r['statut'] ?? '') === 'acceptee') ? '0.5rem' : '1rem' ?>">
            <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:0.5rem;margin-bottom:0.4rem">
                <p style="margin:0;font-weight:<?= in_array((int)$r['id'], $unreadReservationIds, true) ? '800' : '700' ?>;font-size:0.9rem;color:var(--primary);flex:1;min-width:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;display:flex;align-items:center;gap:0.35rem">
                    ...
                    <?php endif; ?>
                    <?= $e($r['expert_titre'] ?? $r['demande_titre'] ?? 'Commande #' . $r['id']) ?>
                </p>
        ...
        <a href="<?= $baseUrl . $bp ?>/payer/<?= (int)$r['id'] ?>" class="client-payment-inline client-payment-inline--compact" role="button" style="margin:0 1rem 1rem;display:flex">
            <span class="client-payment-inline__label">Paiement requis — sans paiement, pas de mission</span>
